  ...
  return (
    <div className="container mx-auto px-4 sm:px-8 py-20 max-w-5xl min-h-[80vh]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {POSTS.map((post) => (
          <Card key={post.id} className="flex flex-col hover:border-emerald-500/50 transition-colors cursor-pointer">
            <CardHeader>
              <div className="flex items-center justify-between text-xs text-muted-foreground mb-3">
                <span>{post.date}</span>
                <span>{post.readTime}</span>
              </div>
              <CardTitle className="text-xl leading-tight mb-2 hover:text-emerald-600 transition-colors">
