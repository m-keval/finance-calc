...
import { Plus, Calendar, Sparkles, Briefcase, Zap, ShieldCheck } from "lucide-react"
...
export function DateAddCalculator() {
  ...
  const handleQuickAdd = (amount: number, unit: "days" | "weeks" | "months" | "years") => {
    setYears(0)
    setMonths(0)
    setWeeks(0)
    setDays(0)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <Card className="lg:col-span-5 p-6 border-brand-100 dark:border-brand-950 space-y-6">
          <div className="flex items-center gap-2 border-b pb-4">
            ...
            <div>
              <h2 className="font-bold text-foreground">Add to Date</h2>
              <p className="text-xs text-muted-foreground">Add days, weeks, months, or years</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs">
              {[
                { label: "+7 Days", fn: () => handleQuickAdd(7, "days") },
                { label: "+14 Days", fn: () => handleQuickAdd(14, "days") },
                { label: "+30 Days", fn: () => handleQuickAdd(30, "days") },
                { label: "+60 Days", fn: () => handleQuickAdd(60, "days") },
                { label: "+90 Days", fn: () => handleQuickAdd(90, "days") },
                { label: "+6 Months", fn: () => handleQuickAdd(6, "months") },
                { label: "+1 Year", fn: () => handleQuickAdd(1, "years") },
                { label: "+5 Years", fn: () => handleQuickAdd(5, "years") },
              ].map((item) => (
                <button
                  key={item.label}
                  type="button"
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <ResultCard
              title="Year Progress"
              value={`${daysRemainingInYear} Days Left`}
              subValue={isLeapYear(result.getFullYear()) ? "Leap Year (366d)" : "Standard Year (365d)"}
              variant="returns"
            />
          </div>
          ...
          <Card className="p-6 border-border/70 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-foreground">Calculation Overview</span>
              <span className="text-xs font-mono text-muted-foreground">
                {businessDaysOnly ? "Working Days Added" : "Calendar Days Added"}
              </span>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-muted/40 border border-border/70 text-center space-y-1">
              ...
              <div className="text-xs text-muted-foreground font-medium">
                Target date calculated from {formatDateDisplay(start)}
              </div>
            </div>
