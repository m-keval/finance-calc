      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-6 space-y-4">
          <Card className="border-none bg-card/50 backdrop-blur-sm">
            <CardContent className="p-4 space-y-4">
              ...
              <NumberInput label={`NECK (${unitSystem === 'metric' ? 'CM' : 'IN'})`} value={neck} onChange={setNeck} min={unitSystem === 'metric' ? 20 : 8} max={unitSystem === 'metric' ? 100 : 40} step={1} prefix="" suffix={unitSystem === 'metric' ? " cm" : " in"} />
              <NumberInput label={`WAIST (${unitSystem === 'metric' ? 'CM' : 'IN'})`} value={waist} onChange={setWaist} min={unitSystem === 'metric' ? 30 : 12} max={unitSystem === 'metric' ? 200 : 80} step={1} prefix="" suffix={unitSystem === 'metric' ? " cm" : " in"} />
              {gender === 'female' && (
                <NumberInput label={`HIP (${unitSystem === 'metric' ? 'CM' : 'IN'})`} value={hip} onChange={setHip} min={unitSystem === 'metric' ? 30 : 12} max={unitSystem === 'metric' ? 200 : 80} step={1} prefix="" suffix={unitSystem === 'metric' ? " cm" : " in"} />
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <ResultCard title="BODY FAT %" value={`${percentage.toFixed(1)}%`} highlight />
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-muted/50 text-xs text-muted-foreground border border-border/50">
            ...
            <p>Calculated using the U.S. Navy Method. Measure waist at navel, neck below larynx. For females, measure hips at widest point.</p>
          </div>
        </div>
      </div>
    </div>
